...
tar_data <- 3 # mean: 1, median: 2, mean/area: 3, median/area: 4
...
x <- list(data_1$c_value/1000, data_2$c_value/1000, 
		data_3$c_value/1000, data_4$c_value/1000) 
y <- list(data_1$ARIMA/1000, data_2$ARIMA/1000, 
		data_3$ARIMA/1000, data_4$ARIMA/1000)


title <- c("(a) '구' 단위 매매", "(b) '구' 단위 전세", 
		"(c) '동' 단위 매매", "(d) '동' 단위 전세")

x.label <- c('', 'ARIMA 추정가 (천만원)')
y.label <- c('실거래가 (천만원)','')
d.lim <- c(6, 3, 18, 6)
...
y.tick = seq(0, d.lim[ik], length=4)
...
text(y.tick[2]*0.9, y.tick[3]*1.4, sprintf("RMSE = %.4f",rmse), cex = cx)
