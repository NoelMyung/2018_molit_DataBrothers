...
price_type <- '전세' # '매매', '전세'
#tar_col <- 2 # mean:1, median:2, mean/area:3, median/area:4
...
dong_fn <- paste('TBL_Mod_서울시_동별_', rest_type, '_', price_type, '.csv', sep='')

value_type <- c('Mean', 'Median', 'MeanPA', 'MedianPA')
out_name <- paste('TSD', rest_type, price_type, value_type[tar_col], sep='_')
...
loc_name <- paste(dong_data$c_si[1], gu_name, dong_name)

tar_data <- dong_data %>% filter(c_dong == dong_name) %>%
...
colnames(tar_data)[tar_col+4] <- 'c_value'

tar_ts <- tar_data %>%
	right_join(time_seq, by = 'c_date') %>%
	select(c_date, c_si, c_gu, c_dong, c_value, is_na)
