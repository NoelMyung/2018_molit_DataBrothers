...
file_name <- paste('TSG', rest_type, price_type, data_type[tar_data],
			 value_type[tar_value], sep='_')
fig_name <- paste('서울시', rest_type, price_type, '구단위_시계열_모델_검정', 
			data_type[tar_data], sep='_')
fig_name <- paste(fig_name, '.tif', sep='')

# read data
data <- as.tibble(fread(paste(file_path, file_name, '.csv', sep = '')))


tiff(paste(fig_path, fig_name, sep='/'), 
